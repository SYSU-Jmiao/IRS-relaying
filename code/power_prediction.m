...
p_predict_result = zeros(length(Rmin_all),K);
p2_predict_result = zeros(length(Rmin_all),K);
W_all = zeros(length(Rmin_all),K);
W2_all = zeros(length(Rmin_all),K);
W = 1/K*ones(K,1);
%% Stage 1
f0 = @(x) (ei(x)-log(x)-eulergamma*ones(size(x))).*exp(-x);
N_all_2 = double(1./(f0(vpa(N_all))));
for rate_i = 1:length(Rmin_all)
    ...
    for i = 1:K
        ...
        f = @(x) rateExpectation(W0(i),x,R_all(i), alpha, noise_power, Wmax)/(Rmin*N_all_2(i))-1;
        P0(i) = fzero(f,1);
    end
    ...
    W = W0;
    P = P0;
    [W,P] = MNVO_Allocation([W;P],R_all,N_all_2,alpha, noise_power, Rmin, Wmax);
    W_all(rate_i,:) = W';
    p_predict_result(rate_i,:) = P';
    tic
    [W2,P2,hist(rate_i)] = ADMM_MNVO_Allocation([W0;P0],R_all,N_all_2,alpha, noise_power, Rmin, Wmax);
    W2_all(rate_i,:) = W2';
    p2_predict_result(rate_i,:) = P2';
    toc;
    %%
    %% Stage 2
    for i = 1:K
